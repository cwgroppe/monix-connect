...
@InternalApi
private[secretsmanager] object AsyncClientConversions { self =>

  def fromMonixAwsConf(monixAwsConf: MonixAwsConf): SecretsManagerAsyncClient = {
    val builder =
      SecretsManagerAsyncClient.builder().credentialsProvider(monixAwsConf.credentials).region(monixAwsConf.region)
  ...
  def from(
    ...
    httpClient: Option[SdkAsyncHttpClient]): SecretsManagerAsyncClient = {
    val builder = SecretsManagerAsyncClient.builder().credentialsProvider(credentialsProvider).region(region)
